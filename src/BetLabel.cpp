...
BetLabel::BetLabel(std::string text, std::string fontName, int fontSize, SDL_Color colour, glm::vec2 position, int fontStyle, bool isCentered) :
...
void BetLabel::setText(std::string newText)
{
...
void BetLabel::setColour(SDL_Color newColour)
{
	FontManager::Instance()->load(m_fontPath, m_fontID, m_fontSize, m_fontStyle);
	FontManager::Instance()->textToTexture(m_text, m_fontID, m_fontID, newColour);
}

void BetLabel::setSize(int newSize)
{
	m_fontSize = newSize;
